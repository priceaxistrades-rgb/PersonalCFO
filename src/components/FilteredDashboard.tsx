...
export function FilteredDashboard({
  ...
  // Calculate net worth from filtered data
  ...
  const invGrowth = invCurrent - invInvested;
  ...
  // Check if completely empty (new user)
    ...
    <>
        ...
        <Card className="!p-3 mb-4" >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium" style={{ color: "var(--text)" }}>
              Showing data for: {selectedNames.join(", ")}
            </span>
          </div>
        </Card>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
        ...
        <KpiCard
          ...
          value={inr(expense, { compact: true })}
          icon="🧾"
          tone="danger"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
        ...
        <KpiCard
          ...
          value={inr(invGrowth, { compact: true })}
          icon="📈"
          tone="primary"
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card title="Income vs Expenses" subtitle={hasSelection ? "Filtered view" : "Last 6 months"} className="lg:col-span-2">
          <LineChart
        ...
        <Card title="Financial Health Score" subtitle={hasSelection ? "Based on filtered data" : "Overall"}>
            ...
            <div className="relative w-40 h-40">
              ...
              <div className="absolute inset-0 grid place-items-center">
                <div className="text-center">
                  <p className="text-3xl font-bold" style={{ color: "var(--text)" }}>{score}</p>
                  <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>out of 100</p>
                </div>
              </div>
            </div>
            <div className="mt-4 w-full space-y-2 text-xs">
              <ScoreRow label="Savings rate" value={`${savingsRate.toFixed(0)}%`} ok={savingsRate >= 20} />
              <ScoreRow label="Emergency cover" value={`${emergencyMonths.toFixed(1)} months`} ok={emergencyMonths >= 6} />
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card title="Spending by Category" subtitle={hasSelection ? "Filtered" : "This month"}>
          {catData.length ? (
            <DonutChart data={catData} centerLabel="Total" centerValue={inr(expense, { compact: true })} />
          ) : (
            <p className="text-sm py-10 text-center" style={{ color: "var(--text-faint)" }}>
              {hasSelection ? "No expenses for selected members" : "No data yet"}
            </p>
          )}
        </Card>
              ...
              return (
                <div key={g.id}>
                  <div className="flex items-center justify-between mb-1.5 text-sm">
                    <span className="font-medium flex items-center gap-2" style={{ color: "var(--text)" }}>
                  ...
                  <div className="flex justify-between mt-1 text-[11px]" style={{ color: "var(--text-faint)" }}>
                    <span>{inr(num(g.saved), { compact: true })}</span>
                    <span>{inr(num(g.target), { compact: true })}</span>
                  </div>
                </div>
              );
            })}
      ...
      <div className="grid lg:grid-cols-3 gap-4">
            ...
            {upcomingBills.map((b) => {
              ...
              return (
                <Tr key={b.id}>
                  ...
                  <Td muted>
                    {fmtDate(b.dueDate)}{" "}
                    <span style={{ color: dleft < 3 ? "var(--danger)" : "var(--text-faint)" }}>
                      ({dleft < 0 ? `${-dleft}d late` : `${dleft}d`})
                    </span>
                  </Td>
                  <Td right strong>{inr(num(b.amount))}</Td>
                  <Td right>
                    <BillToggle id={b.id} paid={b.paid} />
                  </Td>
                </Tr>
              );
            })}
        ...
        <Card title="EMI Summary" subtitle="Active loans">
          <BarChart
            data={debts.map((dt) => ({ label: dt.type.replace("Loan", ""), value: num(dt.emi) }))}
          />
          <div className="mt-3 pt-3 border-t flex justify-between text-sm" style={{ borderColor: "var(--border)" }}>
            ...
            <span className="font-bold" style={{ color: "var(--text)" }}>{inr(totalEmi)}</span>
          </div>
        </Card>
      </div>
    </>
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <span className="font-semibold flex items-center gap-1.5" style={{ color: ok ? "var(--success)" : "var(--warning)" }}>
        <span className="w-1.5 h-1.5 rounded-full" style={{ background: ok ? "var(--success)" : "var(--warning)" }} />
        {value}
      </span>
    </div>
  );
